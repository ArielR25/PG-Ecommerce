...
export default function Producto({
  ...
  function renderFav() {
    ...
    if (stock === "whishlist" && url === "http://localhost:3000/whishlist") {
      return (
        <IoIosCloseCircle
          className="eliminar_fav"
          onClick={() => dispatch(deleteWhishlist(id, token))}
        />
      );
    }
    if (
      ...
      return (
        <button
          className="add_fav_red"
          onClick={() => dispatch(postWhishlist(id, token))}
        >
          ♥
        </button>
      );
  ...
  return (
    <div className="libro">
      ...
      <NavLink
        ...
        className="libro_link"
        to={`/details/${id}`}
      >
        <div className="producto_descuento">{promo ? <p>Oferta</p> : null}</div>
        <div className="producto">
          ...
          {stock >= 0 ? (
            <div>
              <p className="precio">
                <span className="peso">$:</span> {precio}
              </p>
            </div>
          ) : (
            stock !== "whishlist" && (
              <div className="vacio">No hay unidades disponibles</div>
            )
          )}
        </div>
      </NavLink>
