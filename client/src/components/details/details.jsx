...
export default function Details() {
  ...
  if (promo.length > 0) {
    promo.map((e) => e.fechaInicio.split("T")[0]);
  }
      ...
      promo.forEach((p) => {
        p.genero.forEach((genero) => {
          details.generos.forEach((gen) => {
            if (gen === genero) {
              if (
                date >= p.fechaInicio.split("T")[0] &&
                date <= p.fechaFinal.split("T")[0]
              ) {
                promoValidate = {
                  validate: true,
      ...
      <div className="details">
        <div className="detalles_izq">
          <img
            ...
            src={img}
            alt={`imagen de portada del libro: ${titulo}`}
          />
        ...
        <div className="contenido_details">
          <div className="comprar">
            {a && a.admin ? (
              ...
              <button
                className={stock <= 0 ? "vacio_detail" : "comprar_detail"}
                onClick={comprarBoton}
              >
                Comprar
              </button>
            )}
            ...
            {promoValida.validate === false ? (
              ...
              <div>
                <div className="precios_descuento">
                  ...
                  <p className="precio_promo">
                    {Math.round(
                      precio - precio * (promoValida.descuento / 100)
                    )}
                  </p>
                </div>
                <p className="precio_numero_tachado">$ {precio}</p>
                <p className="ahorro">
                  Ahorras: ${Math.round(precio * (promoValida.descuento / 100))}
                </p>
              </div>
            )}
          </div>
          <div className="stock">
            <p className={stock <= 0 ? "stock_vacio" : "stock_unidad"}>
              {stock <= 0
          ...
          <div className="opiniones">
            <h2 className="titulo_valoracion">
