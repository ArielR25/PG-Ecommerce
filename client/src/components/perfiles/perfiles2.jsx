...
import { getProfile2, profileUpdate } from "../../Actions/index";
...
export default function Perfil2() {
  ...
  const state = useSelector((state) => state.profile2);
  const [foto, setfoto] = useState("");
  const [admin, setadmin] = useState("");

  useEffect(() => {
    dispatch(getProfile2(id));
  }, [dispatch, id]);
  ...
  function guardar() {
    token.admin
      ? dispatch(profileUpdate(id, { admin }))
      : dispatch(profileUpdate(id, { foto }));
    setadmin("");
    setfoto("");
  }
      ...
      <div className="userContenedor">
        <div>
          ...
          {token.admin && admin.length === 0 && (
            <Select
              ...
              options={
                !state.admin
                  ? [{ value: "true", label: "Nombrar Administrador" }]
                  : [{ value: "false", label: "Nombrar Usuario" }]
              }
        ...
        <div className="table">
          <div className="childTable">
            ...
            <p className="email_perfil">
              E-Mail: <p>{state.email}</p>
            </p>
            <p className="rol_perfil">
              Rol: {state.admin ? <p>Administrador</p> : <p>Usuario</p>}
            </p>
            {token.admin && (
              <button
                className="eliminar_usuario_perfil"
